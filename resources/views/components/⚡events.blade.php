...
new class extends Component
{
    ...
    {
        $this->events = Event::orderBy('start_date', 'asc')
            ->get()
            ->map(function ($event) {
                return [
                    'image'   => $event->image
                        ? asset('storage/' . $event->image)
                        : '/assets/img/news-1.jpg', // fallback placeholder
                    'title'   => $event->title,
                    'excerpt' => $event->description
                        ? Str::limit($event->description, 150)
                        : '',
                    'date'    => ($event->venue_name ? $event->venue_name . ' . ' : '')
    ...
    {
        return (int) ceil(count($this->events) / $this->perPage);
    }

    public function getPaginatedPostsProperty()
    {
        $offset = ($this->currentPage - 1) * $this->perPage;
        return array_slice($this->events, $offset, $this->perPage);
    }
};
?>
...
<div class="w-full bg-[#f8f5ee] py-16 px-4 sm:px-6 lg:px-8" style="font-family: 'EB Garamond', serif;">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ url('/events') }}" wire:navigate class="text-lg font-serif text-gray-800 hover:text-[#c9a227] transition-colors ml-6 whitespace-nowrap">
                See all
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            @forelse ($this->paginatedPosts as $event)
                <div class="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 group hover:shadow-md transition-shadow duration-300">
                    ...
                    <div class="p-5 pt-2">
                        ...
                        <div class="flex flex-col gap-3">
                            <a href="{{ route('events.show', $event['slug']) }}" wire:navigate class="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-gray-900 hover:text-[#c9a227] transition-colors">
                                Read Full Brief
